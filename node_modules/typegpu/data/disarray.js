...
export const disarrayOf = comptime(((elementType, elementCount) => {
    if (elementCount === undefined) {
        return (count) => cpu_disarrayOf(elementType, count);
    }
    return cpu_disarrayOf(elementType, elementCount);
})).$name('disarrayOf');
export function cpu_disarrayOf(elementType, elementCount) {
    ...
    const disarraySchema = (elements) => {
        if (elements && elements.length !== elementCount) {
            throw new Error(`Disarray schema of ${elementCount} elements of type ${elementType.type} called with ${elements.length} argument(s).`);
        }
