...
import { readFloat16, writeFloat16 } from "./float16Conversion.js";
const dataWriters = {
    bool() {
...
export function writeData(output, schema, value) {
    if (ArrayBuffer.isView(value)) {
        const src = value;
        const expected = sizeOf(schema);
        if (src.byteLength !== expected) {
            logger.warn('suspicious', `TypedArray size mismatch: schema expects ${expected} bytes, got ${src.byteLength}. ` +
...
const dataReaders = {
    ...
    mat3x3f(input) {
        ...
        return mat3x3f(input.readFloat32(), input.readFloat32(), skipOneAfter(), 
        //
        input.readFloat32(), input.readFloat32(), skipOneAfter(), 
        //
        input.readFloat32(), input.readFloat32(), skipOneAfter());
    },
    mat4x4f(input) {
        return mat4x4f(input.readFloat32(), input.readFloat32(), input.readFloat32(), input.readFloat32(), 
        //
        input.readFloat32(), input.readFloat32(), input.readFloat32(), input.readFloat32(), 
        //
        input.readFloat32(), input.readFloat32(), input.readFloat32(), input.readFloat32(), 
        //
        input.readFloat32(), input.readFloat32(), input.readFloat32(), input.readFloat32());
...
export function readData(input, schema) {
...
export function calculateOffsets(options, schema, data) {
    const bufferSize = sizeOf(schema);
    const startOffset = options?.startOffset ?? 0;
    let naturalSize = undefined;
    ...
    const naturalEndOffset = naturalSize !== undefined ? Math.min(startOffset + naturalSize, bufferSize) : undefined;
    const endOffset = options?.endOffset ?? naturalEndOffset ?? bufferSize;
    return { startOffset, endOffset };
}
export function writeToArrayBuffer(buffer, schema, data, options) {
    ...
    if (data instanceof ArrayBuffer || ArrayBuffer.isView(data)) {
        ...
        if (src.byteLength !== regionSize) {
            logger.warn('suspicious', `Buffer size mismatch: expected ${regionSize} bytes, got ${src.byteLength}. ` +
    ...
    if (compiledWriter) {
        try {
            ...
            logger.warn('fallback', `Error when using compiled writer for data type '${schema.type}' (${getName(schema) ?? 'unnamed'}) - this is likely a bug, please submit an issue at https://github.com/software-mansion/TypeGPU/issues\nUsing fallback writer instead.`, error);
        }
    }
...
export function readFromArrayBuffer(buffer, schema) {
