...
export function getImageSourceDimensions(source) {
...
export function clearTextureUtilsCache(device) {
...
function validateBlitFormat(device, format, operation) {
    ...
    if (!isFloat && !isUnfilterableFloat) {
        throw new Error(`Cannot ${operation} for format '${format}': only float formats are supported.`);
    }
    if (!info.canRenderAttachment) {
        throw new Error(`Cannot ${operation} for format '${format}': format does not support render attachments.`);
    }
    return {
        filterable: isFloat,
...
export function generateTextureMipmaps(device, texture, baseMipLevel = 0, mipLevels) {
...
export function resampleImage(device, targetTexture, image, layer = 0) {
    ...
    encoder.copyTextureToTexture({ texture: renderTexture }, { texture: targetTexture, origin: { x: 0, y: 0, z: layer } }, {
