...
function resolveDisarray(ctx, disarray) {
    const element = ctx.resolve(isAttribute(disarray.elementType)
        ? formatToWGSLType[disarray.elementType.format]
        : disarray.elementType).value;
 ...
 */
export function resolveData(ctx, data) {
    if (isLooseData(data)) {
    ...
    if (data.type === 'decorated') {
        return ctx.resolve(data.inner).value;
    }
    if (data.type === 'ptr') {
        if (data.addressSpace === 'storage') {
            return `ptr<storage, ${ctx.resolve(data.inner).value}, ${data.access === 'read-write' ? 'read_write' : data.access}>`;
        }
        return `ptr<${data.addressSpace}, ${ctx.resolve(data.inner).value}>`;
    }
    if (data.type === 'abstractInt' ||
