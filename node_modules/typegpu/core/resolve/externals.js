 ...
 */
export function mergeFunctionExternals(fnExternals) {
    ...
    for (const flatExternal of [fnExternals.args, fnExternals.out].filter((e) => e !== undefined)) {
        for (const [key, value] of Object.entries(flatExternal)) {
            if (key in result && result[key] !== value) {
                throw new Error(`Key '${key}' appears in externals despite already being used for argument/return type. Please rename this external.`);
            }
            result[key] = value;
        }
    }
...
export function addArgTypesToExternals(implementation, argTypes, core) {
...
export function addReturnTypeToExternals(implementation, returnType, core) {
...
export const boundedPropChain = new RegExp(`(?<![\\p{XID_Continue}\\$.])${anyPropChain.source}(?![\\p{XID_Continue}\\$])`, 'ug');
/**
 ...
 */
export function replaceExternalsInWgsl(ctx, externalMap, wgsl) {
    ...
    return wgsl.replaceAll(boundedPropChain, (match) => {
        ...
        if (!Object.hasOwn(externalMap, chain.at(0))) {
            // this prop access does not start with an external
            return match;
        }
        let currentItem = externalMap;
        let suffix = '';
        for (const [i, elem] of chain.entries()) {
            currentItem = currentItem[elem];
            ...
            if (typeof currentItem !== 'object' || currentItem === null || i === chain.length - 1) {
                logger.warn('external-omitted', `During resolution, the external '${chain.slice(0, i + 1).join('.')}' has been omitted. Only TGPU resources, 'use gpu' functions, primitives, and plain JS objects can be used as externals.`);
                return match;
            }
        }
