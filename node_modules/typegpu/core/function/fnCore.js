...
import { isWgslData, isWgslStruct, Void } from "../../data/wgslTypes.js";
...
import { replaceExternalsInWgsl, mergeFunctionExternals, } from "../resolve/externals.js";
import { ResolvableString } from "../resolve/resolvableString.js";
import { extractArgs } from "./extractArgs.js";
export function createFnCore(implementation, functionType, workgroupSize) {
    /**
    ...
    const core = {
        ...
        setExternals(key, newExternal) {
            ...
            if (externals.userProvided && externals.pluginProvided) {
                throw new Error("Cannot call '$uses' on functions whose metadata was provided by unplugin-typegpu.");
            }
        },
        resolve(ctx, argTypes, returnType, entryInput) {
            ...
            if (typeof implementation === 'string') {
                ...
                if (entryInput) {
                    for (const arg of entryInput.positionalArgs) {
                        ...
                        if (!result.success) {
                            throw new Error(`Invalid argument name "${arg.schemaKey}"${result.error ? `: ${result.error}` : ''}`);
                        }
                        if (ctx.isIdentifierBanned(arg.schemaKey)) {
                            throw new Error(`Invalid argument name "${arg.schemaKey}", the identifier is a reserved keyword.`);
                        }
                    }
                    this.setExternals('args', {
                        in: Object.fromEntries(entryInput.positionalArgs.map((a) => [
                            a.schemaKey,
                ...
                if (functionType !== 'normal' && entryInput) {
                    ...
                    const output = returnType !== Void
                        ? isWgslStruct(returnType)
                            ...
                            : ` -> ${attributes !== '' ? attributes : '@location(0)'} ${ctx.resolve(returnType).value} `
                        : ' ';
                    header = `${input}${output}`;
                    body = replacedImpl;
                }
                    ...
                    if (providedArgs.args.length !== argTypes.length) {
                        throw new Error(`WGSL implementation has ${providedArgs.args.length} arguments, while the shell has ${argTypes.length} arguments.`);
                    }
                    const input = providedArgs.args
                        .map((argInfo, i) => `${argInfo.identifier}: ${checkAndReturnType(ctx, `parameter ${argInfo.identifier}`, argInfo.type, argTypes[i])}`)
                        .join(', ');
                    const output = returnType === Void
                        ? ' '
            ...
            if (!ast) {
                throw new Error("Missing metadata for tgpu.fn function body (either missing 'use gpu' directive, or misconfigured `unplugin-typegpu`)");
            }
...
function checkAndReturnType(ctx, name, wgslType, jsType) {
    ...
    if (resolvedWgslType !== resolvedJsType) {
        throw new Error(`Type mismatch between TGPU shell and WGSL code string: ${name}, JS type "${resolvedJsType}", WGSL type "${resolvedWgslType}".`);
    }
    return wgslType;
}
