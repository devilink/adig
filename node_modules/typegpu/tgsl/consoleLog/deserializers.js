...
import { bitcast } from "../../std/bitcast.js";
import { unpack2x16float } from "../../std/packing.js";
import { logger } from "../../tgpuLogger.js";
const toF = (n) => bitcast(u32, f32)(n ?? 0);
const toI = (n) => bitcast(u32, i32)(n ?? 0);
...
const deserializerMap = {
    ...
    mat3x3f: (d) => 
    // oxfmt-ignore
    mat3x3f(toF(d[0]), toF(d[1]), toF(d[2]), toF(d[4]), toF(d[5]), toF(d[6]), toF(d[8]), toF(d[9]), toF(d[10])),
    mat4x4f: (d) => 
    // oxfmt-ignore
    mat4x4f(toF(d[0]), toF(d[1]), toF(d[2]), toF(d[3]), toF(d[4]), toF(d[5]), toF(d[6]), toF(d[7]), toF(d[8]), toF(d[9]), toF(d[10]), toF(d[11]), toF(d[12]), toF(d[13]), toF(d[14]), toF(d[15])),
};
// -------
// Helpers
// -------
...
function deserialize(data, dataType) {
    ...
    if (isWgslArray(dataType)) {
        ...
        const result = deserializeCompound(data, Array.from({ length }, () => elementType));
        return result;
    }
...
export function deserializeAndStringify(serializedData, argTypes) {
 ...
 */
export function logDataFromGPU(resources) {
    ...
    void indexBuffer.read().then((totalCalls) => {
        if (totalCalls > options.logCountLimit) {
            logger.warn('log-limit-exceeded', `Log count limit per dispatch (${options.logCountLimit}) exceeded by ${totalCalls - options.logCountLimit} calls. Consider increasing the limit by passing appropriate options to tgpu.init().`);
        }
    });
