...
for (const [name, serializer] of Object.entries(serializerMap)) {
    serializer
        .$name(`serialize${name[0].toLocaleUpperCase()}${name.slice(1)}`)
...
function getSerializer(dataType, dataBuffer) {
    ...
    if (maybeSerializer) {
        return maybeSerializer.with(dataBufferSlot, dataBuffer);
    }
 ...
 */
export function createLoggingFunction(id, dataTypes, dataBuffer, indexBuffer, logOptions) {
    const serializedSize = dataTypes.map(sizeOf).reduce((a, b) => a + b, 0);
    if (serializedSize > logOptions.logSizeLimit) {
        throw new Error(`Logged data needs to fit in ${logOptions.logSizeLimit} bytes (one of the logs requires ${serializedSize} bytes). Consider increasing the limit by passing appropriate options to tgpu.init().`);
    }
