import { blankSpaces, lineBreaks } from "./core/whitespaces.js";
...
function isSpaceRequired(current, next) {
    if (next === undefined) {
        return false;
    }
    const currentLast = current.at(-1);
    const nextFirst = next.at(0);
    invariant(currentLast?.length === 1 && nextFirst?.length === 1, `Expected tokens during minification to not be empty.`);
...
function stripWGSLComments(code) {
    ...
    while (offset < code.length) {
        ...
        if (code.startsWith('/*', offset)) {
            ...
            if (depth > 0) {
                throw new SyntaxError(`Block comment opening without corresponding closing found during minification.`);
            }
            copiedUpTo = offset;
            continue;
        }
        if (code.startsWith('*/', offset)) {
            throw new SyntaxError(`Block comment closing without corresponding opening found during minification.`);
        }
        offset += 1;
    }
