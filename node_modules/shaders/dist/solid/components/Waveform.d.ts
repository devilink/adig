import { JSX } from 'solid-js';
import { BlendMode, PropDriver, TransformConfig, BoundingBoxConfig, LayoutConfig } from '../../core';
import { ComponentProps } from '../../core/shaders/Waveform';
export type { ComponentProps };
/**
 * Base props that all shader components have
 */
interface BaseShaderProps {
    children?: JSX.Element;
    blendMode?: BlendMode;
    opacity?: number;
    visible?: boolean;
    id?: string;
    maskSource?: string;
    maskType?: string;
    renderOrder?: number;
    transform?: Partial<TransformConfig>;
    boundingBox?: Partial<BoundingBoxConfig>;
    flow?: LayoutConfig;
    absolute?: boolean;
}
/**
 * Component-specific props that merge base props with shader-specific props
 * Note: ComponentProps are made optional since they have defaults from the shader definition
 */
type ShaderComponentProps = BaseShaderProps & Partial<Omit<ComponentProps, 'from' | 'to' | 'amplitude' | 'frequency' | 'height' | 'count' | 'barWidth' | 'rounding' | 'dotSize' | 'lineWidth' | 'softness'>> & {
    from?: ComponentProps['from'] | PropDriver;
    to?: ComponentProps['to'] | PropDriver;
    amplitude?: ComponentProps['amplitude'] | PropDriver;
    frequency?: ComponentProps['frequency'] | PropDriver;
    height?: ComponentProps['height'] | PropDriver;
    count?: ComponentProps['count'] | PropDriver;
    barWidth?: ComponentProps['barWidth'] | PropDriver;
    rounding?: ComponentProps['rounding'] | PropDriver;
    dotSize?: ComponentProps['dotSize'] | PropDriver;
    lineWidth?: ComponentProps['lineWidth'] | PropDriver;
    softness?: ComponentProps['softness'] | PropDriver;
};
/**
 * The main Solid wrapper component for Shader shader nodes
 */
export default function ShaderComponent(props: ShaderComponentProps): JSX.Element;
//# sourceMappingURL=Waveform.d.ts.map